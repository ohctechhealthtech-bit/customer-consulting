  ...
  return (
    <div className="min-h-screen bg-[#f5f8fb]">
      ...
      <footer className="w-full px-4 py-8 text-center text-[10px] font-bold uppercase tracking-widest text-slate-400">
        © 2026 OHCTECH · Patient Consent & Data Collection
      </footer>
    </div>
  );
}
        ...
        return (
          <li key={label} className="flex flex-1 items-center">
            <div className="flex items-center gap-3">
              <div
                className={`grid h-9 w-9 shrink-0 place-items-center rounded-full text-xs font-bold transition-all ${done
              ...
              <div
                className={`hidden text-xs font-semibold sm:block ${active || done ? "text-foreground" : "text-muted-foreground"
                  }`}
              >
                {label}
              </div>
            ...
            {i < steps.length - 1 && (
              <div
                className={`mx-3 hidden h-px flex-1 sm:block ${done ? "bg-brand-gradient" : "bg-slate-200"
                  }`}
              />
            )}
          </li>
