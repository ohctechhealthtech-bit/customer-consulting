...
function PatientDashboard() {
  ...
  const chartData = useMemo(() => {
    // Last 30 days map
    const dataMap: Record<string, { date: string, consentCount: number, loginCount: number, profileCount: number }> = {};
    const now = new Date();
    for (let i = 29; i >= 0; i--) {
    ...
    if (profileHistory) {
      profileHistory.forEach((group: any) => {
                ...
                <CardContent className="p-6">
                  <div className="flex items-center justify-between gap-6">
                    <div className="flex items-center gap-5 min-w-0">
                      <div className={`grid h-12 w-12 shrink-0 place-items-center rounded-xl ${isAccepted ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-500'}`}>
                        {isAccepted ? <BadgeCheck className="h-7 w-7" /> : <XCircle className="h-7 w-7" />}
                      </div>
                      <div className="min-w-0">
                        <div className="text-[11px] font-bold text-muted-foreground uppercase tracking-[0.12em] leading-none mb-2">Health Information Consent</div>
                        <div className="flex items-center gap-2">
                          <span className={`text-xl font-bold tracking-tight ${isAccepted ? 'text-slate-800' : 'text-amber-700'}`}>
                    ...
                    <div className="shrink-0">
                      {isAccepted ? (
                        ...
                        <BrandButton onClick={() => handleConsentAction("ACCEPT")} className="h-10 px-6 text-[11px] font-bold">
                          Grant Consent
                        </BrandButton>
                      )}
                    </div>
                ...
                <CardContent className="p-5">
                  ...
                  >
                    <div className="text-left">
                      ...
                      <p className="text-[11px] text-muted-foreground font-medium">Update account credentials</p>
                    </div>
                    <ChevronRight size={16} className="text-slate-400 group-hover:text-emerald-600 transition-all group-hover:translate-x-0.5" />
                  </button>
                </CardContent>
              </Card>
            </div>
            ...
            <div className="lg:col-span-7">
              <Card className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden h-full">
                <CardHeader className="border-b border-slate-100 flex flex-row items-center justify-between px-6 py-4">
                  ...
                  <BrandButton variant="outline" size="sm" onClick={handleEditClick} disabled={updating} className="h-8 px-4 text-[11px] font-bold">
                  ...
                  <dl className="grid gap-x-12 gap-y-6 sm:grid-cols-2">
                    ...
                    <div className="sm:col-span-2 mt-2 pt-6 border-t border-slate-50">
                      <div className="flex items-center gap-4 p-4 bg-slate-50/50 rounded-xl border border-slate-100">
                        ...
                        <div>
                          <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest block mb-0.5">Employment Verified</span>
              ...
              <Card className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden flex flex-col h-[350px]">
                ...
                <CardContent className="p-0 overflow-y-auto flex-1">
                  <div className="divide-y divide-slate-100">
                    {logins?.map((l: any) => (
                      <div key={l.id} className="flex gap-4 items-center p-5 hover:bg-slate-50 transition-colors">
                        <div className="grid h-9 w-9 place-items-center rounded-lg bg-slate-50 border border-slate-100 text-slate-400 shrink-0">
                          <Clock size={16} />
                        </div>
                        <div className="min-w-0">
                          <p className="text-[13px] font-bold text-slate-700 truncate">{new Date(l.login_time).toLocaleString([], { dateStyle: 'medium', timeStyle: 'short' })}</p>
                          <p className="text-[11px] text-muted-foreground font-semibold truncate uppercase tracking-tighter">{l.browser || "System"} • {l.device_type || "PC"}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                      ...
                      <div key={idx} className="p-6 hover:bg-white transition-all">
                        <div className="max-w-2xl">
                          <div className="flex items-center gap-3 mb-4">
                            ...
                            <div>
                              <p className="text-[11px] font-black text-slate-800 uppercase tracking-tight">Profile Updated</p>
                              <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest mt-0.5">
                                {new Date(group.updatedAt).toLocaleString(undefined, { dateStyle: 'medium', timeStyle: 'short' })}
                              </p>
                            </div>
                          </div>
                          ...
                          <div className="space-y-3 pl-5 border-l border-slate-100 ml-2.5">
                            {group.changes.map((ch) => (
                              <div key={ch.id} className="group/item">
                                <div className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1.5">{ch.fieldName}</div>
                                <div className="flex items-center gap-3 bg-slate-50/80 px-3 py-2 rounded-lg border border-slate-100/50 w-fit">
                                  <span className="text-[11px] text-slate-400 line-through truncate max-w-[120px]">{ch.oldValue || "—"}</span>
                                  <ChevronRight size={10} className="text-slate-300 shrink-0" />
                                  <span className="text-[11px] font-bold text-emerald-600 truncate max-w-[180px]">{ch.newValue || "—"}</span>
                                </div>
                              </div>
                            ))}
                          </div>
                ...
                <CardContent className="p-0 overflow-y-auto flex-1">
                  <div className="divide-y divide-slate-100">
                    {(consent?.history || []).length === 0 && (
                      <div className="py-20 text-center text-[13px] text-muted-foreground font-medium italic">No activity recorded.</div>
                    )}
                    {consent?.history?.map((h: any) => (
                      <div key={h.id} className="flex items-center justify-between p-5 hover:bg-slate-50 transition-colors">
                        <div className="flex items-center gap-4 min-w-0">
                          <div className={`grid h-9 w-9 place-items-center rounded-full shrink-0 ${h.action === 'ACCEPT' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'}`}>
                            {h.action === 'ACCEPT' ? <Check className="h-4 w-4" /> : <AlertCircle className="h-4 w-4" />}
                          </div>
                          <div className="min-w-0">
                            <p className="text-[13px] font-bold text-slate-800 uppercase tracking-tight truncate">{h.action}</p>
                            <p className="text-[11px] text-muted-foreground font-bold mt-0.5 uppercase tracking-tighter truncate opacity-70">
                              {new Date(h.performed_at).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}
                            </p>
                          </div>
                        </div>
                        <span className="text-[10px] font-bold text-slate-400 bg-slate-50 px-2 py-1 rounded-md border border-slate-100 uppercase tracking-widest shrink-0">Verified</span>
                      </div>
                    ))}
                  </div>
                </CardContent>
